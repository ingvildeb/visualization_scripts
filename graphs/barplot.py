...
GROUPING_HIERARCHY_LEVEL = "CustomLevel5_gm"
FINE_GM_LEVELS_FOR_GROUP_LABELS = {
    "CustomLevel1_gm",
    "CustomLevel2_gm",
    "CustomLevel3_gm",
}
GROUP_LABEL_OVERRIDES = {
    "fiber tracts": "fiber tracts and ventricular systems",
    "ventricular systems": "fiber tracts and ventricular systems",
}


def add_parent_group_axis(ax, parent_labels):
    ax.set_xticklabels([])
...
ontology, bundle, id_mapping, color_mapping, _acronym_mapping, _hierarchy_regions = prepare_hierarchy_info_atlaslevels()
parent_label_mapping = build_parent_label_mapping(bundle, selected_hierarchy, GROUPING_HIERARCHY_LEVEL)
...
for file in files:
    ...
    if region_list:
        region_ids = [ontology.resolve_name(region) if isinstance(region, str) else region for region in region_list]
        values_in_file = collect_values_directly(data_file, value_column, region_ids)
    else:
...
for region_id, value in value_dict.items():
    ...
    parent_label = parent_label_mapping.get(region_id)
    parent_labels.append(GROUP_LABEL_OVERRIDES.get(parent_label, parent_label))
    values.append(value)
...
if selected_hierarchy in FINE_GM_LEVELS_FOR_GROUP_LABELS and not specified_parent:
    add_parent_group_axis(ax, parent_labels)
